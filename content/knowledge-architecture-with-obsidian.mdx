...
For years I kept my notes in a single directory of Markdown files, edited entirely in VS Code. It was functional, but there was a problem: it couldn’t keep pace with my thinking. My ideas evolved, ramified, and recombined, while the directory itself remained static. Even though I made the filenames descriptive (`tpf-migration-master-note.md`, `tpf-archive-deployment-process.md`), the flat directory provided no organization. Notes related to each other only in my head, if at all, and nothing external to my mind supported the connective work of actual thinking: relating ideas, returning to undeveloped ones, and bringing out latent patterns.
