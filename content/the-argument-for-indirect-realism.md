...
draft: false
---
I will call it "The Argument". It assumes many guises and has survived for hundreds of years, and I would like to explain what I think is wrong with it. Here is a classic instance, from David Hume in his 1748 _Enquiry Concerning Human Understanding_:
...
Hume's _argument from perspectival variation_ is just one among many similar arguments which all purport to prove the two-part doctrine of indirect realism: that in perception we are not aware of the things we naively suppose ourselves to be aware of — tables, sunrises, chickens and people — and that the things we actually _are_ aware of are perceptual intermediaries of some sort, mental objects often referred to as "representations" or similar. The venerable arguments from illusion, from double-vision and from hallucination are all versions of The Argument.
...
But surely there _must_ be a diminution when I walk away from the table, if we are to accept the findings of physiologists and psychologists of vision, the significance of optics, and even our own experience. _Something_ is getting smaller.
...
Now it might be wondered how perception can be said to be anything but indirect if we can switch between ways of seeing things while those things do not change. Does this not lend weight to the thesis that what we see are, even if not neutral sense-data or the inferences thereof, then _aspects_ which are constructed by the brain?

I do not think so. Both the duck-aspect and the rabbit-aspect are there in the drawing, in its lines and curves. Nothing is added by the mind that is not already there to be seen. Switching between them is switching between two ways of organizing real features of an external thing, not a shift in which mental object we are aware of. Multistability is a fact about ambiguous external objects, not evidence that ordinary, unambiguous perception involves an intermediary.
