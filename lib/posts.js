...
function myMatter(fileContents) {
    ...
    */
    ...
    if (!Array.isArray(data.tags)) {
        data2 = { ...data, tags: data.tags.split(/[ ,]+/) };
    }    

    let contentWithImgUrls = content.replaceAll('/bucket/', `${config.imageLocation}/tr:w-${config.bodyImage.width},q-${config.bodyImage.quality}/`);
...
export async function getPost(slug) {
    const fullPath = path.join(config.contentDirectory, `${slug}.mdx`);
    ...
    const excerpt = contentHtml.split(' ').slice(0, 38).join(' ');
...
export async function getAllPosts(sortedBy = 'date') {
    ...
    const posts = files.map((filename) => {
        ...
        const fullPath = path.join(config.contentDirectory, filename);
