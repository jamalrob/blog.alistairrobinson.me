...
// Update your markdown-it configuration to allow HTML
const markdownRenderer = md({
  typographer: true,
  html: true,  // This is the key option - enables HTML tags
  linkify: true,
});

function myMatter(fileContents) {
  /* Your existing myMatter function remains the same */
...
export async function getPost(slug) {
  ...
  // Use the markdown renderer that allows HTML
  const contentHtml = markdownRenderer.render(content);

  // Convert HTML → plain text (no tags left)
  const plain = convert(contentHtml, { wordwrap: false });

  let excerpt = plain.split(' ').slice(0, 38).join(' ');

  if (data.image) {
    excerpt = ` <img src="${settings.imageLocation}/tr:w-${settings.thumbnailImage.width},q-${settings.thumbnailImage.quality}/${slug}.${data.imageExtension || 'jpg'}" />${excerpt}`
  }
...
// Also update getPhilArchivePost with the same fix
export async function getPhilArchivePost(slug) {
  const fullPath = path.join(settings.contentDirectoryPhilArchive, `${slug}.mdx`);
  const { data, content } = myMatter(fs.readFileSync(fullPath, 'utf8'));

  const contentHtml = markdownRenderer.render(content); // Use the same renderer
