...
function myMatter(fileContents) {
    /* Returns the frontmatter with the tags property value into an array if it's not already one
    */
...
export async function getPost(slug) {
    ...
    const { data, content } = myMatter(fileContents);
...
export async function getAllPosts(sortedBy = 'date') {
    ...
    const posts = files.map((filename) => {
        ...
        const fileContents = fs.readFileSync(fullPath, 'utf8');
        const { data } = myMatter(fileContents);
